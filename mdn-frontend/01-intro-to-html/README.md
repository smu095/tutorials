...
- Inline elements _will not_ cause a new line to appear in the document.
...
- Elements can have **attributes**, which contain extra information about the element which _is not visible to the user_.
...
4. `<meta charset="utf-8">`: This element specifies the character set for your document to UTF-8. The page can now handle any textual content it might contain.
5. `<title></title>`: This sets the title of the page, which is the title that appears in the browser tab the page is loaded in. The page title is also used to describe the page when it is bookmarked.
6. `<body></body>`: This contains all the content that displays on the page, including text, images, videos, games, playable audio tracks, or whatever else.
...
- The `<em>` (emphasis) element is used to _stress_ certain words.
...
```html
<a
  href="mailto:example@mail.com?cc=other@mail.com&subject=The%20subject%20of%20the%20email&body=The%20body%20of%20the%20email"
>
...
- For cases like these, HTML provides the `<div>` (short for _division_) and `<span>` elements with a suitable `class` attribute, to provide some kind of label for them so they can be easily targeted.
